...
> 2. 用户粘贴 key 后，创建 `user_config.json`（已 `.gitignore` 保护），写入下方 provider 配置块，key 填入 `apiKey`。  
> 3. 确认写入完成，通知用户重启 OpenCode。  
> 
> **方式 B（占位符，不通过对话传 key）**：  
> 1. 创建 `user_config.json`，写入下方 provider 配置块，`apiKey` 字段值写 `<YOUR_GO_API_KEY>`。  
> 2. 告诉用户："文件已创建。打开 `user_config.json`，把 `<YOUR_GO_API_KEY>` 替换成你在 opencode.ai/auth 创建的真实 key，保存后重启 OpenCode。"  
> 3. key 不走对话，用户本地操作。安全政策不再阻止。  
> 
...
> ⚠️ provider 配置在 `user_config.json`（已在 Provider 节创建），**不要**把 key 写入本文件。本文件只含权限和 agent 定义。
...
test -f user_config.json && echo "Provider config ok" || echo "Provider config MISSING"
```

预期：Agent 19，Commands 5，Skills 3，Config ok，Provider config ok。
...
2. 输入 `@工具人` 能正常响应（如果无响应，检查 `user_config.json` 的 `apiKey` 是否已替换真实 key）
