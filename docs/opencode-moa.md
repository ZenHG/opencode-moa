...
> **AI 部署流程**：执行到这里**必须主动询问用户**：  
> "请输入你的 OpenCode Go API Key（在 opencode.ai/auth 创建后复制）。"  
> 用户输入 key 后填入下方 `apiKey` 字段。如果用户没有 key，先引导到 opencode.ai/auth 完成 Go 订阅（首月 $5，之后 $10/月）并创建 key。  
> **不可跳过此步骤或用占位符 `<你的 Go API Key>` 代替**，否则工具层 agent 全部静默失败。
