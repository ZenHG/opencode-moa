...
> 2. 创建目录 `.opencode/local/`，key 写入文件 `.opencode/local/opencode-go.key`（纯文本，只有 key 字符串，无换行）。  
> 3. 在项目 `opencode.json` 中加入下方 provider 配置块，`apiKey` 字段值写 `{file:.opencode/local/opencode-go.key}`。  
> 4. 确认完成，通知用户重启 OpenCode。  
> 
> **方式 B（占位符，不通过对话传 key）**：  
> 1. 创建目录 `.opencode/local/`，写入 `.opencode/local/opencode-go.key`，内容仅占位符 `<YOUR_GO_API_KEY>`。  
> 2. 在项目 `opencode.json` 中加入下方 provider 配置块，`apiKey` 字段值写 `{file:.opencode/local/opencode-go.key}`。  
> 3. 告诉用户："key 文件在 `.opencode/local/opencode-go.key`，打开后把 `<YOUR_GO_API_KEY>` 替换成真实 key（opencode.ai/auth 创建），保存后重启 OpenCode。"  
> 4. key 不走对话，用户本地操作。  
> 
> **不可跳过此步骤**。无论用哪种方式，必须让用户有一个带真实 key 的 `opencode-go` provider，否则 19 个 agent 全部静默失败。

---

**方式 A（推荐）：配置文件直接写 key**

把以下代码段放入项目 `opencode.json`（自动加载，`apiKey` 用 `{file:}` 引用独立 key 文件）或系统级 `~/.config/opencode/opencode.json`（全平台生效，仓库外）：
...
- `@工具人` 能正常响应。
- `pwsh .opencode/tests/T0-static-verify.ps1` → 41 PASS。
...
> ⚠️ **前置检查**：开始部署前，确认已按上方 **「Provider 配置」** 节完成 key 配置（`user_config.json` 已创建且含有效 key 或占位符）。如果还没配，翻回上方先处理，否则 19 个 agent 部署后全连不上。
...
预期：Agent 19，Commands 5，Skills 3，Config ok，Key file ok。
