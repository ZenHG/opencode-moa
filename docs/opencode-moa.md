...
MoA 的 19 个 agent 全部用 `opencode/<model-id>`。要让子代理连得上，**必须有一个已鉴权的 `opencode` provider**（否则内置 provider 无 key 会降级为 `public`，付费 Go 模型被禁用，工具层报 `OpenCode Go provider error` / `Upstream request failed`）。二选一：

**方式 A：`-connect` 官方鉴权（推荐）**

在 OpenCode 内执行 `/connect`，登录你的 OpenCode Go 订阅账号。内置 `opencode` provider 会自动鉴权，无需任何配置。

**方式 B：手动 key 直连（适合 CI / 无交互环境）**

在系统级配置 `~/.config/opencode/opencode.json`（全项目生效，Windows / Linux / macOS 路径相同）加入：
...
> 方式 B 中的 `provider.opencode` 会覆盖内置同名 provider，并复用同一套 `opencode/<model>` 前缀，agent 文件无需改动。

**验证 provider 已生效**

- `/models` 能看到 `deepseek-v4-flash`、`kimi-k2.7-code` 等 Go 模型（非 `Free` 标记）。
- 重启 OpenCode 后 `@工具人` 能正常响应。

> ⚠️ 含 `apiKey` 的配置文件不要提交到仓库。系统级 `~/.config/opencode/` 本就在仓库外；项目级配置请加入 `.gitignore`。
