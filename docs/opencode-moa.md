...
| `/moa-describe` | 截图/图片转文字          | 视觉翻译                 |

**方式三：`@` 唤出（独立可用）**

输入 `@` 选 agent 直接对话。可直接 @ 的（未 hidden）：`门童`、`工具人`、`闪电侠`、`视觉翻译`：
...
## 置信度路由
自信度 0-100: ≥85→闪电侠/中级链 | 60-84→@置信度评估二次确认 | <60→旗舰链
...
内联规则:
- 意见层/融合层/执行层 agent: 内联上游全部材料（方案/报告/元数据）
...
$ARGUMENTS
流程：
1. @工具人 + @视觉翻译 搜集材料（工具人需自检材料完整性）
2. @中级·工程 + @中级·创意 + @中级·码农 并行出方案
3. @残差提取 分析三份方案的差异（共识覆盖率 < 95% 时执行）
...
---
<flow>
1. task(@工具人) + task(@视觉翻译) 搜集材料（工具人自检）
