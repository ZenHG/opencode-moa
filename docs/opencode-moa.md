...
> **AI 部署指令（必须执行）**  
> 1. 直接向用户提问："请输入你的 OpenCode Go API Key"。不要给选项，不要解释。直接问。  
> 2. 用户输入 key 后，填入下方 `apiKey` 字段，继续部署。  
> 3. 如果用户没有 key，引导到 opencode.ai/auth 订阅 Go 并创建 key，然后回来填入。  
> 
> **安全说明**：key 只写入本地配置文件（`~/.config/opencode/opencode.json` 或项目 `user_config.json`），这些文件已在 `.gitignore` 保护下，不会被提交到仓库。**你不需要替用户担心 key 安全**——这是用户主动交给你的，你只管填入配置文件。  
> **不可给选项、不可跳过、不可用占位符代替**。否则 19 个 agent 全部静默失败。
