...
Por defecto, OpenCode utiliza un solo modelo de principio a fin. Cambiar un carácter y diseñar una arquitectura de sistema utilizan el mismo prompt, la misma temperatura, el mismo contexto. Sin división del trabajo.
...
Tres planes independientes de tres modelos diferentes forman naturalmente una estructura de "consenso + divergencia". El modelo de fusión identifica lo que es consenso y lo mantiene, y toma lo mejor donde divergen — algo que un solo modelo no puede hacer.
...
| -------------------- | -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OpenCode instalado    | `opencode --version`             | **>= 1.3.4** (soporte a nivel de agente `reasoningEffort`/`hidden`/`task`; el proveedor `openai-compatible` pasa razonamiento de manera transparente, no se necesita `forceReasoning`), [instalar](https://opencode.ai/install) |
...
> ⚠️ **La ruta de clave a nivel del sistema es fácil de colocar incorrectamente** — ortografía correcta en "Leer antes de desplegar" a continuación. Una ruta incorrecta lleva a que el despliegue parezca exitoso pero todos los agentes no pueden conectarse.

> ⚠️ **Leer antes de desplegar: no coloques mal la ruta de clave**
> Coloca el proveedor + clave en el **`opencode.json` a nivel de proyecto** (predeterminado, autónomo) o en la **ruta compartida a nivel del sistema** — elige **una**.
...
> 
> Nota: este método copia el `.opencode/` empaquetado del repositorio tal cual — sus agentes tienen **nombres de visualización en chino**. Si deseas agentes con nombres en inglés (para que puedas `@nombre-en-ingles`), usa el Método 1 en su lugar.
...
3. Ejecuta el script de verificación: `pwsh .opencode/tests/T0-static-verify.ps1` (generado por el Bloque manual 5.5 durante el despliegue), se espera que todos PASEN (FAIL=0; con clave a nivel de sistema, WARN también cuenta como pase)

### Reversión de un clic
...
| "escribir un módulo de autenticación de usuario" | la capa de herramientas reúne → 3 paralelas de nivel medio → fusionar | manejador de herramientas + trío de nivel medio + fusionar |
| "diseñar una arquitectura de microservicios" | la capa de herramientas reúne → 3 paralelas insignia → fusionar → implementar → QA | 6 agentes de cadena completa             |
| "restaurar la interfaz de esta captura de pantalla" | 3 expertos en frontend paralelos → el líder elige el mejor         | cuarteto de frontend                    |
| mensaje con captura de pantalla         | traductor-visual convierte a texto → enrutamiento normal          | traductor-visual                        |
| mensaje con registro de errores / diagrama / contenido complejo | traductor-visual descompone el contenido → enrutamiento normal  | traductor-visual (rol de respaldo)     |
...
| `/moa-frontend` | restauración de UI, CSS, arreglo de captura de pantalla |
...
```
             ...
             Flash + MiMo         3 opiniones paralelas toman la mejor
             (~80% de llamadas)   (~18% de llamadas)  (~2% de llamadas)
```
...
**Capa de fusión** (Kimi K3 / Qwen-Max / GLM / DeepSeek Pro de respaldo) — mantener el consenso, tomar lo mejor en divergencia, con respaldo a DeepSeek V4 Pro si la fusión falla. La fusión insignia ahora funciona en **Kimi K3** (2.8T params, 1M contexto, modelo de frontera de primer nivel) — empujando el techo de calidad de MoA al frente del grupo.
...
conserje-enrutador (门童路由员, Flash)
 ...
 │   traductor-visual (视觉翻译官, MiMo ) captura de pantalla/UI→texto; registros/diagramas/documentos→descomposición
 │
 ├── extractor de residuos  (残差提取者,  Flash     ) analizar divergencia entre planes
 ├── evaluador de confianza (置信度评估者, DS Pro    ) evaluar la confianza del resultado de la fusión
 │
...
Agente de respaldo (no en la cadena del enrutador anterior, llamado solo cuando la fusión falla):

```
respaldo (融合·保底, DeepSeek V4 Pro) — misma fusión mejorada por residuos, utilizada cuando la fusión insignia / fusión media / líder de frontend fallan

## Diseño de tolerancia a fallos

### Cadena de retroceso de la capa de herramientas

El fallo de la capa de herramientas no congela — se degrada automáticamente:
...
Los agentes de opinión individuales (arquitectura/planificación/ingeniería, restauración de frontend/lógica/movimiento, ingeniería creativa/codificación de nivel medio) pueden devolver resultados vacíos o agotar el tiempo de manera independiente. El sistema maneja esto de manera elegante:
...
> Esto evita que la canalización se detenga (ATASCADO) cuando un agente de opinión falla — una queja común de los usuarios.
...
La activación de condiciones sigue la lógica de cortocircuito: condiciones preestablecidas cumplidas → activar; ninguna cumplida → preguntar al usuario por confirmación. Esto reemplaza las reglas de activación codificadas (como "screenshot disponible → @vision-translator") con precondiciones declaradas por el agente, auto-documentadas.
...
- `Capa de Fusión` — fase de fusión y verificación del plan
- `Capa de Implementación` — fase de implementación de código y aceptación

### Informe de progreso unificado

Tanto los caminos de éxito como de fallo siguen el mismo formato de informe, sin exponer nunca los nombres de los agentes internos:
...
- `stuck` — todos los caminos de recuperación agotados, se necesita intervención del usuario

### Acceso paralelo rápido

Cuando la canalización principal se está ejecutando, swift puede ser despachado en paralelo para subtareas simples independientes:
...
- La instrucción del usuario solicita explícitamente trabajo en paralelo ("hacer X simultáneamente", "también verificar rápidamente Y")
...
- Capa de opinión: `read: deny` + `bash: deny`, solo puede planificar en base al material de la capa de herramientas
- Capa de fusión: misma restricción, solo puede fusionar en base a las tres opiniones
...
### Defensa contra la anidación de tareas

Todos los agentes que no son de enrutamiento declaran `task: deny` para evitar que los agentes secundarios llamen a task() nuevamente, bloqueando la anidación recursiva:

- **Capa 1 (frontmatter del agente)**: cada archivo de agente declara `task: deny`
- **Capa 2 (opencode.json)**: `permission.task` solo permite al concierge llamar a los agentes; los agentes que no son de enrutamiento se les niega globalmente la llamada a los trabajadores
- **Capa 3 (guardia de aviso)**: el aviso del concierge termina con una restricción que prohíbe lanzarse a sí mismo una nueva canalización a través de un sub-agente

> Añadido en 2026-07 después de descubrir la anidación triple concierge→tool-handler→tool-handler. La redundancia de tres capas asegura el bloqueo incluso si una capa falla.
...
MoA factura por una mezcla ponderada por volumen de llamadas: ~80% capa de herramienta Flash, ~18% de gama media, ~2% insignia. Estime el precio unitario efectivo de salida con los precios por unidad en la tabla de costos de esta sección:

> **Importante**: Las proporciones 80/18/2 son **distribución de volumen de llamadas esperada diseñada por la arquitectura**, no proporciones de costo medidas. El uso real depende de los tipos de tareas y la complejidad.
...
El precio unitario efectivo combinado ≈ **$0.72 / 1M**. Comparado con "GLM todo insignia $7.50" → aproximadamente 10% → **~90% ahorrado**; comparado con "DeepSeek Pro todo gama media $3.48" → aproximadamente 21% → **~79% ahorrado**. La afirmación de "ahorrar 90%" es el verdadero valor en comparación con la línea base insignia.
...
> Todos los ID de modelos son solo declaraciones; reemplace con cualquier modelo que prefiera.
...
> ⚠️ Límites de modelos gratuitos: ventana de contexto más pequeña, posible respuesta más lenta, los datos pueden ser utilizados para entrenamiento, gratuito por un tiempo limitado.
...
| Aislamiento de permisos de MCP   | capa de opinión prohibida de leer código (leer: denegar / bash: denegar), previene el eludir la capa de herramienta (el proyecto no tiene servidor MCP configurado; "MCP" aquí se refiere a restricciones de herramientas a nivel de agente) |
...
El repositorio incluye tres scripts de verificación en `.opencode/tests/`. La Capa 0 es completamente automática; las Capas 1–2 son listas de verificación guiadas que se recorren dentro de OpenCode.

```bash
# Capa 0 — verificación estática (automática, 0 token)
pwsh .opencode/tests/T0-static-verify.ps1
# esperado: todo PASS / FAIL=0 (con clave a nivel de sistema, WARN también cuenta como pase)
...
| -------------------------- | ----- | ------------------------------------------------------------------------------------- | -------------------- |
| `T0-static-verify.ps1`    | 0     | Verifica la estructura de archivos, conteos de agente/comando/habilidad, anclajes de README, corrección de ruta clave | Automático           |
| `T1-behavioral-guide.ps1` | 1     | Imprime una lista de verificación paso a paso para comportamiento de enrutamiento / opinión / fusión | Manual (en OpenCode) |
...
A: El Método 1 es el más conveniente: arrastra `docs/opencode-moa.en.md` a la caja de chat y deja que la IA lo despliegue automáticamente. Los Métodos 2/3 requieren operar primero en un terminal (CMD/PowerShell/Terminal).
...
A: Solo cambia el campo `model` del agente:
...
A: Sí. MoA ya está copiado en el directorio `.opencode/` de tu proyecto; el repositorio original puede ser eliminado.

**Q: ¿Cómo despliego en múltiples proyectos?**
A: Despliega cada proyecto por separado. `.opencode/` es una configuración a nivel de proyecto y no afecta a otros proyectos.
...
| `scripts/hooks/pre-commit`  | Recordatorio de gancho git local: advierte cuando haces un cambio en `CHANGELOG.md` (que se libera automáticamente al hacer push a `master`).      |
