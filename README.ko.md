...
> 🔥 **핫 (2026-07):** **DeepSeek-V4-Flash-0731** 정식 버전 출시 — 에이전트 성능이 대폭 강화되어 더 비싼 **GLM-5.2**를 능가 (Terminal Bench 82.7 vs 81.0, DeepSWE 54.4 vs 46.2, Toolathlon 70.3 vs 59.9). 저렴한 모델이 비싼 모델을 이김——MoA의 툴 계층·의견 계층 주력 Flash가 같은 비용으로 크게 강화됐습니다.

> 🔄 **장기 자기 개선 (24h 무인 운영):** 프로젝트를 며칠 동안 자동 반복 — 잊지 않고, 멈추지 않고, 반복하지 않음. 도어맨이 매 라운드 기동하여 MoA 파이프라인 전체를 거쳐 진행 상황을 디스크에 저장. 한 줄 명령으로 시작: **[▶ 시작하기 →](longloop/docs/LongLoop.md)**
...
concierge-router (门童, Flash)
 ...
 │   vision-translator (视觉翻译, Qwen3.7 Plus ) 스크린샷/UI→텍스트; 로그/다이어그램/문서→분해
 │
 ├── 잔여물 추출기  (残差提取,  Flash     ) 계획 간의 분기를 분석
 ├── 신뢰도 평가자 (置信度评估, DeepSeek V4 Flash    ) 융합 결과 신뢰도 평가
 │
 ├── 중간 의견 계층 ─────────────────────────────────────────────
