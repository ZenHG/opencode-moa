...
门童路由员现在根据关键词分析自动检测任务类型：

- **探索型任务**："分析"、"对比"、"理解"、"调查" → 启用探索型 prompt + 冻结验收标准
- **执行型任务**："修复"、"添加"、"实现"、"部署" → 启用执行型 prompt + 止损规则
- **任务类型显示在流水线输出中**：`[Type: Execute]` 或 `[Type: Explore]`
...
| 闪电侠 | always |
| 工具人 | 需要代码库上下文 |
| 视觉翻译官 | 主条件：`截图`；降级条件：`错误日志 OR 图表 OR 长文档 OR 模糊意图` |
| 中级·工程 | 需要工程复杂度 |
| 中级·创意 | 需要创意复杂度 |
| 中级·码农 | 需要实现复杂度 |
...
条件激活遵循短路原则：前置条件满足 → 激活；无任何前置条件满足 → 询问用户确认。这替代了硬编码触发规则（如"有截图→+@视觉翻译官"），变成 Agent 自描述的、可文档化的元数据。
...
```
[流水线] 模式=<lite|balanced|strict>  阶段=<工具层|意见层|融合层|实现层>  状态=<等待中|执行中|已完成|降级|已卡死>
  reason: <为何走该层级>
