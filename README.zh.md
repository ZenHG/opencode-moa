...
| 带日志/图表/复杂内容的消息 | 视觉翻译官解构 → 正常路由     | 视觉翻译官（降级角色） |
...
> 这防止了一条意见链失败时流水线卡死（STUCK）的问题——这也是用户反馈的常见痛点。

### 声明式 Agent 预条件

Agent 的激活由声明式 `precondition` 元数据驱动，而非硬编码路由规则。每个 Agent 声明何时应该被激活：

| Agent | 预条件 |
|-------|--------|
| 闪电侠 | always |
| 工具人 | 需要代码库上下文 |
| 视觉翻译官 | 主条件：`screenshot`；降级条件：`error_log OR diagram OR long_document OR ambiguous_intent` |
| 中级·工程 | 需要工程复杂度 |
| 中级·创意 | 需要创意复杂度 |
| 中级·码农 | 需要实现复杂度 |
...
条件激活遵循短路原则：preconditions 满足 → 激活；无任何 precondition 满足 → 询问用户确认。这替代了硬编码触发规则（如"有截图→+@视觉翻译官"），变成 Agent 自描述的、可文档化的元数据。
...
- `in_progress` — 当前阶段执行中
- `complete` — 阶段成功完成
- `degraded` — 以部分输入降级模式运行
- `stuck` — 所有恢复路径耗尽，等待用户干预
