...
| `hooks/pre-commit` | 本地 git 钩子提醒：暂存 `CHANGELOG.md` 改动时警告（push 到 master 会自动发版）。 |
| `hooks/pre-push` | 本地 git 钩子提醒：push `CHANGELOG.md` 改动到 master 前确认版本；非交互/CI 环境自动放行。 |

> 这些钩子不会自动安装。若想要提醒，把它们软链到 `.git/hooks/`。
