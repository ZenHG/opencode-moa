...
> 🔥 **Chaud (2026-07) :** modèle phare mis à jour vers **Kimi K3** — 2.8T params, 1M contexte, modèle de pointe. Le plafond de qualité de MoA est maintenant en tête du peloton.
...
2. **Goulot d'étranglement de qualité** — un seul modèle a seulement une façon de penser, facilement bloqué dans des angles morts
...
Trois plans indépendants provenant de trois modèles différents forment naturellement une structure de "consensus + divergence". Le modèle de fusion identifie ce qui est consensus et le conserve, et prend le meilleur là où ils divergent — quelque chose qu'un seul modèle ne peut pas faire.
...
| Plan OpenCode Go     | console opencode.ai                 | [Abonnez-vous](https://opencode.ai/auth), premier mois 5 $, puis 10 $/mois                                                                                                                                 |
...
> Pas de pwsh/jq, pas de problème — vous pouvez utiliser la Méthode 1 (déploiement automatique par IA) ou la Méthode 3 (fusion manuelle).
...
> ⚠️ **Le chemin de clé au niveau système est facile à placer incorrectement** — orthographe correcte dans "Lire avant de déployer" ci-dessous. Un mauvais chemin entraîne un déploiement semblant réussir mais tous les agents échouent à se connecter.
...
>   Un mauvais chemin au niveau système entraîne "le déploiement réussit mais tous les agents ne peuvent pas se connecter".
...
# exécuter le script d'installation (fusion automatique de la config, conserve votre clé API)
# Windows :
...
> 
> Remarque : cette méthode copie le `.opencode/` inclus du dépôt tel quel — ses agents ont **des noms d'affichage chinois**. Si vous souhaitez des agents avec des noms en anglais (pour pouvoir `@english-name`), utilisez plutôt la Méthode 1.
...
Pour échanger un modèle, modifiez cette seule ligne dans `.opencode/agents/<agent>.md` avec n'importe quel `provider/model-id` auquel vous avez accès (par exemple, `opencode-go/kimi-k2.7-code`, `opencode-go/glm-5.2`). Pas de réinstallation nécessaire. Mélangez et associez librement — le modèle ne vous lie à rien.
...
cp -r opencode-moa/.opencode/ votre-projet/
cp -r opencode-moa/.moa/ votre-projet/
...
1. Après avoir redémarré OpenCode, appuyez sur `Tab` pour faire défiler les agents (client de bureau Windows : `Ctrl+.` fonctionne également) et voir "concierge-router"
2. Tapez `@tool-handler` et il répond
3. Exécutez le script de vérification : `pwsh .opencode/tests/T0-static-verify.ps1` (généré par le Bloc 5.5 manuel lors du déploiement), attendu tous PASS (FAIL=0 ; avec clé au niveau système, WARN compte également comme pass)

### Rétrogradation en un clic

```bash
rm -rf votre-projet/.opencode/
rm -rf votre-projet/.moa/
...
| "écrire un module d'authentification utilisateur" | la couche d'outils rassemble → 3 intermédiaires parallèles → fusion | gestionnaire d'outils + trio intermédiaire + fusion |
| "concevoir une architecture de microservice" | la couche d'outils rassemble → 3 phares parallèles → fusion → mise en œuvre → QA | chaîne complète de 6 agents          |
| "restaurer l'interface utilisateur de cette capture d'écran" | 3 experts frontend parallèles → le leader choisit le meilleur      | quatuor frontend                     |
...
| `/moa-medium`    | module fonctionnel, correction de bogue, refactorisation de fichier unique |
...
Le concierge-routeur détecte maintenant automatiquement le type de tâche en fonction de l'analyse des mots-clés :
...
**Couche d'opinion** (MiniMax / DeepSeek Pro / Qwen / MiMo-Pro) — plans de différentes perspectives. Trois opinions forment naturellement une structure de "consensus + divergence".
...
Les agents d'opinion et de fusion utilisent des marqueurs `---nom-section---`. Couche d'opinion : `---mémoire---` + `---plan---` + `---PAS---`. Couche de fusion : structure complète avec métadonnées, consensus, plan, liste de PAS et critères d'acceptation. Permet le parsing en aval et la vérification des acceptations figées.

### Anti-triche (v0.0.15)

Empêche les agents d'implémentation de couper les coins ronds : non-régression de base, actions interdites (sauter/moquer/supprimer des tests), contrôles aléatoires cachés, vérification des différences d'implémentation, stop-loss (3 tentatives par élément, retour en arrière en cas de régression). Modèle de critères d'acceptation à `.moa/acceptance-template.json`.
...
concierge-routeur (门童路由员, Flash)
 ...
 │   gestionnaire-d'outils-mimo (工具人-mimo, MiMo) [caché]  lecture de fichier fiable (secours + parallèle)
 │   swift                      (闪电侠, Flash    ) tâches simples en une seule fois
 │   traducteur visuel          (视觉翻译官, MiMo ) capture d'écran/UI→texte ; journaux/diagrammes/docs→décomposition
 │
 ├── extracteur de résidu  (残差提取者,  Flash     ) analyser la divergence entre les plans
 ...
 │   flag-impl (旗舰·实现, Flash) [caché]  mettre en œuvre par plan fusionné
 ...
 └── Couche d'opinion frontend ─────────────────────────────────────────────
     ...
     fe-logic   (前端·逻辑, Qwen3.7 Plus) architecture de composant & gestion d'état
...
```
tool-handler (Flash) a échoué → nouvelle tentative immédiate une fois
  → nouvelle tentative réussie → retour normal
  → nouvelle tentative échoue → tool-handler-mimo (MiMo) a échoué → nouvelle tentative immédiate une fois
    → nouvelle tentative réussie → retour normal
    → nouvelle tentative échoue → demander à l'utilisateur :
      A. attendre quelques minutes et réessayer
      B. ignorer la couche outil, appeler directement la couche opinion (coût plus élevé)
...
Si l'agent de fusion principal échoue (BLOQUÉ / ERREUR_FOURNISSEUR / délai d'attente / résultat vide), le concierge-routeur rétrograde automatiquement vers `@融合·保底` (DeepSeek V4 Pro, sauvegarde) :

```
flag-fuse (旗舰·融合, Kimi K3) a échoué
  → tâche(@融合·保底) (DeepSeek V4 Pro) → sortie du résultat de sauvegarde
mid-fuse (中级·融合, Kimi) a échoué
  → tâche(@融合·保底) (DeepSeek V4 Pro) → sortie du résultat de sauvegarde
fe-lead (前端·总工, GLM-5.2) a échoué
  → tâche(@融合·保底) (DeepSeek V4 Pro) → sortie du résultat de sauvegarde
```

L'agent de secours utilise le même processus de fusion amélioré par résidu.

### Tolérance aux pannes partielles de la couche opinion

Les agents d'opinion individuels (architecture/planification/ingénierie, restauration-frontend/logique/mouvement, ingénierie intermédiaire/créatif/codage) peuvent renvoyer des résultats vides ou expirer indépendamment. Le système gère cela avec grâce :

```
3 agents d'opinion parallèles dispatchés
  → tout agent renvoie un résultat vide → réessayer cet agent une fois
    → nouvelle tentative réussie → continuer normalement
    → nouvelle tentative échoue → marquer comme "dégradé" et procéder avec N/3 entrées
      → 残差提取者 fonctionne uniquement avec les entrées disponibles
...
- Les perspectives manquantes sont étiquetées `[Manquant : nom de la perspective]`
- La couverture de consensus < 50 % déclenche un avertissement de "fusion dégradée à faible confiance"
- La fusion à source unique (N=1) applique un facteur de pénalité de confiance de 0,7

> Cela empêche le pipeline de se bloquer (BLOQUÉ) lorsqu'un agent d'opinion échoue — une plainte courante des utilisateurs.
...
|-------|---------------|
| 闪电侠 | toujours |
| 工具人 | nécessite un contexte de code source |
| 视觉翻译官 | principal : `screenshot`; secours : `error_log OU diagram OR long_document OU ambiguous_intent` |
...
```
[Étape : Couche Outil] → [Étape : Couche Opinion] → [Étape : Couche Fusion] → [Étape : Couche Implémentation]
```
...
- `Couche Fusion` — phase de fusion et de vérification de plan
- `Couche Implémentation` — phase d'implémentation de code et d'acceptation

### Rapport de progrès unifié

Les chemins de succès et d'échec suivent le même format de rapport, n'exposant jamais les noms internes des agents :

```
[Pipeline] mode=<lite|balanced|strict>  stage=<Couche Outil|Couche Opinion|Couche Fusion|Couche Implémentation>  status=<inactif|en_cours|complet|dégradé|bloqué>
  raison : <pourquoi cette étape>
  chemin : <Couche Outil|Chaîne intermédiaire|Chaîne phare|Chaîne frontend>
  secours : <stratégie de récupération>
```

Indicateurs de statut :
- `en_cours` — exécution de l'étape actuelle
- `complet` — étape terminée avec succès
- `dégradé` — fonctionnement avec des entrées partielles, confiance réduite
- `bloqué` — tous les chemins de récupération épuisés, intervention de l'utilisateur nécessaire
...
```
Pipeline principal : Couche Outil → Couche Opinion → Couche Fusion → Couche Implémentation
Voie parallèle : swift (toujours prêt, fonctionne aux côtés du pipeline principal)
```
...
Si swift termine avant le pipeline principal, les résultats sont conservés et retournés ensemble à la fin. Si le pipeline principal termine en premier, les résultats de swift sont retournés immédiatement. L'échec de swift n'affecte pas l'exécution du pipeline principal.
...
> Remarque : Ce projet ne configure aucun serveur MCP. Le terme "isolement des permissions MCP" fait référence aux restrictions des outils au niveau des agents (`read: deny` / `bash: deny`), et non à l'isolement au niveau des serveurs MCP.

### Défense contre la nidification des tâches

Tous les agents non-routiers déclarent `task: deny` pour empêcher les agents enfants d'appeler à nouveau task(), bloquant ainsi la nidification récursive :

- **Couche 1 (frontmatter de l'agent)** : chaque fichier d'agent déclare `task: deny`
- **Couche 2 (opencode.json)** : `permission.task` ne permet que le concierge d'appeler des agents ; les agents non-routiers sont globalement interdits d'appeler des travailleurs
...
> Ajouté le 2026-07 après avoir découvert la nidification triple concierge→tool-handler→tool-handler. La redondance à trois couches garantit le blocage même si une couche échoue.
...
- `ERREUR_FOURNISSEUR` — serveur 502/503/délai d'attente
- `ERREUR_AUTH` — échec d'authentification
- `ERREUR_INCONNU` — autres erreurs
...
MoA facture par un mélange pondéré en fonction du volume d'appels : ~80% outil de couche Flash, ~18% intermédiaire, ~2% phare. Estimez le prix unitaire de sortie effectif avec les prix par unité dans le tableau des coûts de cette section :
...
Prix unitaire de sortie effectif mélangé ≈ **0,72 $ / 1M**. Comparé à "tous les modèles phare GLM 7,50 $" → environ 10% → **~90% économisé** ; comparé à "tous les modèles intermédiaires DeepSeek Pro 3,48 $" → environ 21% → **~79% économisé**. La revendication "économiser 90%" est la véritable valeur par rapport à la référence phare.
...
| Isolation des permissions MCP   | la couche d'opinion est interdite de lecture de code (lecture : refus / bash : refus), empêche de contourner la couche d'outil (le projet n'a pas de serveur MCP configuré ; "MCP" fait référence aux restrictions d'outils au niveau de l'agent) |
| Défense à 3 couches        | les agents non routables refusent la tâche → liste blanche de conciergerie → garde de prompt, empêche l'imbrication récursive                                                                 |
| Chaîne de repli           | la couche d'outil échoue → demande à l'utilisateur → attendre/sauter/modèle gratuit                                                                                                            |
| Rétrogradation en un clic  | supprime `.opencode/` pour restaurer                                                                                                                                                         |
...
# attendu : tout PASS / FAIL=0 (avec clé au niveau système, WARN compte également comme pass)

# exécutez les trois couches à la fois
...
A: Non. Le script d'installation ne fusionne que les configurations `permission`, `agent`, `default_agent` de MoA, en gardant vos `provider`, `model`, etc. existants. Le fichier original est automatiquement sauvegardé sous `.bak.timestamp`.
...
A: La Méthode 1 est la plus pratique — faites glisser `docs/opencode-moa.en.md` dans la boîte de chat et laissez l'IA déployer automatiquement. Les Méthodes 2/3 nécessitent de fonctionner d'abord dans un terminal (CMD/PowerShell/Terminal).

### Usage

**Q: Je ne vois pas "concierge-router" ?**
A: Vérifiez les trois vérifications sous "déploiement en 30 secondes → Comment savoir si le déploiement a réussi" : `opencode.json` à la racine du projet, 22 .md sous `.opencode/agents/`, changez avec `Tab` après le redémarrage (client de bureau Windows : `Ctrl+.` fonctionne aussi).
...
A: Appuyez sur `Tab` pour changer (client de bureau Windows : `Ctrl+.` fonctionne aussi), ou tapez `/build`, `/plan`. MoA n'affecte pas les agents intégrés.
...
A: Voir "Conception de tolérance aux pannes → Chaîne de secours" ci-dessus : MoA demande à l'utilisateur de choisir A. attendre quelques minutes / B. ignorer la couche d'outils et appeler directement la couche d'opinion (coût plus élevé).

**Q: Où sont les modèles gratuits ?**
A: Voir "Coût → Modèles gratuits" ci-dessus : utilisez `/models` pour ouvrir la liste des modèles et en choisir un étiqueté "Gratuit" (client de bureau Windows : `Ctrl+'` fonctionne aussi) (DeepSeek V4 Flash Free, MiMo-V2.5 Free, North Mini Code Free, etc.). Les modèles gratuits ont un contexte limité, peuvent être plus lents, et les données peuvent être utilisées pour l'entraînement.
...
| `scripts/hooks/pre-commit` | Rappel de hook git local : avertit lorsque vous mettez en scène un changement dans `CHANGELOG.md` (qui se libère automatiquement lors de la poussée vers `master`).                                   |
| `scripts/hooks/pre-push`   | Rappel de hook git local : confirme la version avant de pousser les changements de `CHANGELOG.md` vers `master` ; procède automatiquement dans des environnements non interactifs/CI. |

> Ces hooks ne sont pas installés automatiquement. Créez un lien symbolique dans `.git/hooks/` si vous souhaitez les rappels, par exemple `ln -s ../../scripts/hooks/pre-push .git/hooks/pre-push`.
