...
あなた: メッセージキュー設計を手伝って
    ...
    ├─ flag-plan (GLM        ) ─── PM視点の案
    ├─ flag-eng  (MiniMax M3 ) ─── 実装者視点の案
    └─ flag-fuse (Qwen3.7 Max) ─── それぞれの長所を統合し、最適案にする
```
...
混合後の有効出力単価は ≈ **$0.69 / 1M** です。"all-flagship GLM $7.50" と比べると約9% → **約90%節約**。"all-mid-tier DeepSeek Pro $3.48" と比べると約20% → **約80%節約**。"save 90%" は flagship baseline に対する実際の価値です。
...
Deploy 後、static check を実行します（`pwsh` が必要）:
